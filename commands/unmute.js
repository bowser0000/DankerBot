module.exports = {
    ...
    execute(message) {
        ...
        if (!taggedUser.roles.cache.some(role => role.name === 'Muted')) return message.reply(`${taggedUser.user.tag} is already unmuted!`);
        async function unmuteRole() {
            if (!muteRole) return message.reply('this person is not muted!');
            async function removeMutedRole() {
                await (taggedUser.roles.remove(muteRole.id));
            }
            removeMutedRole();
